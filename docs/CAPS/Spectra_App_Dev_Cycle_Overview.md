...
_Reference-driven plan aligned to your Refined Spec Brain Dump (Canvas)_
...
- User workflows (short stories).
- UI requirements (controls, labels, what persists, what can reset).
- Data expectations (inputs, common messiness, how to resolve).
- Behavior rules (MUST/MUST NOT, trust constraints).
- Failure modes (user-facing messages + next steps).
- Provenance + notes (what is captured and displayed).
- Export requirements (what is added to export bundles).
- Acceptance tests (concrete checks).
- Open questions (defer without losing track).
